<template>
  <div id="app">
    ...
    <div class="direita">
      ...
      <a href="https://expo.dev/client" target="blank"
        >Baixe o app cliente aqui
      </a>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      id: "",
      msg: "",
      dependencias: `{"expo-status-bar":{"version":"~1.0.4"}}`,
      codigo: `
 ...
 `,
  ...
  methods: {
    ...
    atualizarCodigo: async function() {
      ...
      const { id } = await this.snack.saveAsync();
    ...
    atualizarDependencias: async function() {
      ...
      const { dependencies } = await this.snack.getStateAsync();
      var erro = false;
      for (const name in dependencies) {
        if (dependencies[name].handle === undefined) {
          erro = true;
          alert("Não foi possível resolver a dependencia " + name);
          console.log(dependencies[name].handle); // string -> https://...
        }
      }
